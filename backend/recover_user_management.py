...
file_path = r"c:\Projects\Adstra_Digital\adstra-next\src\components\admin_side\UserManagement\UserManagement.jsx"

with open(file_path, "r", encoding="utf-8") as f:
    content = f.read()

# 1. Add LEAD_PERMISSION_SECTIONS and buildPermissionSections
prefix = """const BASE_URL = API_BASE_URL;
const FULL_ACCESS_ROLES = new Set(["admin", "super_admin"]);"""

new_prefix = """const BASE_URL = API_BASE_URL;
const FULL_ACCESS_ROLES = new Set(["admin", "super_admin"]);
const MODULE_LABELS = {
...
state_block = """  const [roles, setRoles] = useState({});
  const [permissions, setPermissions] = useState({});"""
new_state_block = """  const [roles, setRoles] = useState({});
  const [permissions, setPermissions] = useState({});
...
handle_add = """  const handleAddUser = () => {
    setEditUser(null);
    setShowForm(true);
  };"""
new_handle_add = """  const handleAddUser = () => {
    setEditUser(null);
    setSelectedRoleForm("employee");
    setShowForm(true);
  };"""
content = content.replace(handle_add, new_handle_add)

handle_edit = """  const handleEdit = (user) => {
    setEditUser(user);
    setShowForm(true);
  };"""
new_handle_edit = """  const handleEdit = (user) => {
    setEditUser(user);
    setSelectedRoleForm(user.role || "employee");
    setShowForm(true);
  };"""
content = content.replace(handle_edit, new_handle_edit)
...
content = content.replace(role_select, new_role_select)

# 5. Replace permissions-panel
old_permissions_panel = """                <div className="permissions-panel">
...
new_permissions_panel = """                <div className="permissions-panel">
...
content = content.replace(old_permissions_panel, new_permissions_panel)
