...
const oldPreviewStart = '        {/* Traditional Header */}'; // Already updated once to "Traditional Header"
const oldPreviewEnd = '        {/* Footer */}';

const newBoxedLayout = `        {/* Boxed GST Style Invoice Content */}
        <div style={{ border: '2px solid black', fontFamily: 'Arial, sans-serif', color: 'black', background: 'white' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid black' }}>
            <div style={{ padding: '8px', borderRight: '1px solid black', fontSize: '11px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <tbody>
                  <tr><td style={{ width: '40%', padding: '2px 0' }}>Invoice Number</td><td style={{ width: '5%' }}>:</td><td style={{ fontWeight: 'bold' }}>{invoice?.invoice_no}</td></tr>
                  <tr><td style={{ padding: '2px 0' }}>Invoice Date</td><td>:</td><td>{invoice?.date ? new Date(invoice.date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : "—"}</td></tr>
                  <tr><td style={{ padding: '2px 0' }}>Due Date</td><td>:</td><td>{invoice?.due_date ? new Date(invoice.due_date).toLocaleDateString('en-IN') : "Upon Delivery"}</td></tr>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: '1px solid black' }}>
            <div style={{ padding: '8px', borderRight: '1px solid black', fontSize: '11px' }}>
              ...
              <div>GSTIN: {invoice?.client?.gstin || "-"} | Mobile: {invoice?.client?.contact || "-"}</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
              ...
              <tfoot>
                <tr style={{ borderTop: '1px solid black', fontWeight: 'bold' }}>
                  ...
                  <td style={{ padding: '6px', textAlign: 'right' }}>{Number(invoice?.discount_amount || 0).toFixed(2)}</td>
                </tr>
                <tr style={{ borderTop: '1px solid black', fontWeight: 'bold', backgroundColor: '#f3f4f6' }}>
                  <td colSpan="6" style={{ borderRight: '1px solid black', padding: '6px', textAlign: 'right' }}>Total (Incl. Taxes)</td>
                  <td style={{ padding: '6px', textAlign: 'right', fontSize: '14px' }}>{Number(invoice?.total_amount || 0).toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
          </div>
...
if (startIndex !== -1 && endIndex !== -1) {
  ...
  console.log("Success: InvoicePreview.js updated with Boxed Layout.");
} else {
