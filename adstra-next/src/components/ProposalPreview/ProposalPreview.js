...
export default function ProposalPreview({ services = [], sections: propSections = [] }) {
  ...
  // Generate dynamic sections for each service category
  ...
  // Merge dynamic sections with propSections
    ...
    <div className="proposal-preview-wrapper text-[15px] text-gray-800 leading-relaxed">
      ...
      <div className="pdf-page">
        ...
        {services.length > 0 && (
          <div className="border-t mt-3">
            <h4 className="text-xl font-bold text-indigo-800">💼 Services In Brief</h4>
            <div className="w-full overflow-x-auto">
              <table className="w-full text-sm border border-gray-800 shadow-md rounded overflow-hidden">
                <thead>
                  <tr>
                    {["Description", "Qty", "Rate", "GST", "Amount"].map((col, i) => (
                      <th
                        key={i}
                        style={{
                ...
                <tbody>
                  {Object.entries(groupedServices).map(([category, items], groupIdx) => (
                    <React.Fragment key={groupIdx}>
                      <tr className="bg-gray-200 text-gray-700" style={{backgroundColor: "lightgrey"}}>
                        <td colSpan={5} className="px-4 py-2 font-semibold border border-gray-300">
                          {category}
                        </td>
                      </tr>
                      {items.map((item, idx) => {
                        ...
                        return (
                          ...
                          >
                            <td className="px-4 py-2 border border-gray-200">{item.description}</td>
                            <td className="px-4 py-2 border border-gray-200 text-center">{qty}</td>
                            <td className="px-4 py-2 border border-gray-200 text-right text-indigo-700 font-medium">
                              ₹{rate.toFixed(2)}
                            </td>
                            <td className="px-4 py-2 border border-gray-200 text-center">₹{gstAmount.toFixed(2)}</td>
                  ...
                  <tr className="bg-indigo-100 text-indigo-800 font-semibold text-sm">
                    <td colSpan={4} style={{ fontSize: "0.8rem", paddingLeft: "5%" }} className="footer-small py-3 text-left border border-gray-300 uppercase">
                      <b>Amount in Words:</b> ₹ {numberToWords(total)}
                    </td>
                    <td className="px-4 py-3 text-right border border-gray-300">₹{total.toFixed(2)}</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <div className="pdf-page" style={{ breakBefore: "page" }}>
        {sections.map((sec, i) => {
          ...
          return (
            <div key={i} className="mb-8 pb-6 border-b border-dashed border-gray-300 last:border-none last:pb-0">
              <h4 className="text-xl sm:text-2xl font-bold text-indigo-700 mb-2" style={{ textAlign: sec.alignment || "left" }}>
                {sec.title}
              </h4>
              <div className="text-[14px] sm:text-[15px] text-gray-800 leading-relaxed space-y-2" style={{ textAlign: sec.alignment || "left" }}>
                {contentElements}
              </div>
            </div>
          );
      ...
      <div className="mt-6 p-4 text-center rounded-md bg-gray-50">
        <h4 className="text-lg font-semibold text-gray-800 mb-2">🏦 We bank with HDFC Bank</h4>
        <p>
          <strong>Account Holder:</strong> Adstra Digital Pvt. Ltd. <br />
          <strong>A/C No:</strong> 50200091927202 | <strong>IFSC:</strong> HDFC0001595
        </p>
      </div>

      {/* --- Conclusion --- */}
      <div className="mt-6 mb-2 text-gray-700 leading-relaxed text-center">
        <h4 className="text-xl font-semibold mb-2 text-indigo-700">Conclusion</h4>
        ...
        <div style={{ fontSize: "0.7rem" }} className="footer-small">
          <p>
            <strong>LUT Registered:</strong> hence ZERO taxation for Overseas billing.
            <br />© {new Date().getFullYear()} <strong>Adstra Digital</strong>. All rights reserved.
            <br />Powered by Passion • Driven by Strategy • Delivered with Creativity
          </p>
        </div>
      </div>
    </div>
